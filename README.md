...
| Functions  | Usage |
| ------------- | ------------- |
| ```javascript has(type)```  | Parameters tha can be  used to find a digit in a string: 'numbers', 'digits' or 'num'.  
...
| ```javascript hasString(string)```  | The parameter ```string``` is used to as an input to insert a string.  
For example:  
```javascript
